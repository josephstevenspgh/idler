function init_ingame()
	...
	fadetimer = 0
	gameactive = true
	sp_exclamation = sprite_init("null", gfx_exclamation, gfx_exclamation:getWidth(), gfx_exclamation:getHeight(), {0, 0, 0, 0})
	...
	--current map
   	...
   	foglayer = false
   	fogheight = 5
   	gfx_fog = love.graphics.newImage("fog1.png")
   	...
   	--ui sprites
   	local barwidth = 25
	local mfgbar = bar_init(gfx_bar_blue_small, barwidth, 0, 1, 1, 1)
	local mbgbar = bar_init(gfx_bar_background_small, barwidth, barwidth, 2, 1, 2)
	local barwidth = 75
	local hfgbar = bar_init(gfx_bar_red, barwidth, 0, 1, 1, 1)
	local hbgbar = bar_init(gfx_bar_background, barwidth, barwidth, 2, 1, 2)
	local ebar = bar_init(gfx_bar_orange_tiny, barwidth, 0, 1, 1, 1)
	local ebar2 = bar_init(gfx_null, 1, 0, 1, 1, 1)
	player_expbar = healthbar_init(ebar, ebar2)
	player_cooldownbar = healthbar_init(mfgbar, mbgbar)
	player_healthbar = healthbar_init(hfgbar, hbgbar)
	--level up background
	levelup_bg = init_textbox_art(26, 12)
end
...
function draw_ingame()
	...
	love.graphics.origin()
	if foglayer then
		love.graphics.draw(bottomlayer)
		love.graphics.setBlendMode("add")
		love.graphics.setColor(0, 100, 255, 255)
		camera_center(sp_player)
		love.graphics.draw(gfx_fog, 16, 16)
		love.graphics.origin()
	...
	healthbar_changewidthp(player_healthbar, sp_player.hp/sp_player.maxhp)
	healthbar_changewidthp(player_cooldownbar, sp_player.cooldown/sp_player.maxcooldown)
	healthbar_changewidthp(player_expbar, sp_player.exp/sp_player.exptable[sp_player.level])
	healthbar_draw(player_healthbar, 5, 5)
	healthbar_draw(player_cooldownbar, 5, 25)
	healthbar_draw(player_expbar, 5, 16)
	...
	if dolevelup then
		--show level up menu
		love.graphics.draw(levelup_bg, 48, 32)
		local sx = 48+8
		local sy = 32+8
		...
		lprint("level up!", sx, sy)
		lprint("increase a stat", sx, sy+10)
		lprint("attack\nagility\ndefence", sx+8, sy+20)
		love.graphics.setColor(0, 255, 0, 255)
		lprint("+"..gain[0].."\n+"..gain[1].."\n+"..gain[2], sx+70, sy+20)
		love.graphics.setColor(255, 255, 255, 255)
		draw_cursor(sx, sy+20+menuposy*8)
		lprint("new ability!", sx, sy+60)
		showicon(1, sx, sy+70, "ability")
		lprint("cut - bushes bout to\nget fucked up", sx+20, sy+70)
	elseif talking then
		--show textbox
		...
		lprint(textbox.displaytext, sx+8, sy+8)
		--questions
		if textbox_question then
			...
			if string.len(textbox.text) == string.len(textbox.displaytext) then
				draw_cursor(sx+16, sy+16+menuposy*8)
			end
		end
	end
...
function draw_player()
	...
	if cantakeaction(sp_player) then
		sp_exclamation.x = sp_player.x - 1
		sp_exclamation.y = sp_player.y - 6
...
function make_damage_num(sprite, damage, color)
	...
	love.graphics.setColor(255, 255, 255, 255)
   	love.graphics.setFont(font_classic)
	local tempart = love.graphics.newImage(numart:newImageData())
...
function dospecial(n)
	...
	elseif n == 102 then
		--shop
		maketextbox("hero", "hello, welcome to shop shop!\nBuy my shit!")
		makequestionbox("buy things?", {"sure", "no way"})
		questionbox.answer[0] = function() 
			make_shop(1)
			talking = false
		end
...
function player_controls(sprite)
	...
	elseif sprite.state == 300 then
		...
		if not sprite.abilityready then
			...
			if sprite.cooldown >= sprite.maxcooldown then
...
function changemap(id)
	...
	toplayer = love.graphics.newCanvas(currentmap.width, currentmap.height)
	tmpcanvas = love.graphics.newCanvas(currentmap.width, currentmap.height)
	bottomlayer = love.graphics.newCanvas(currentmap.width, currentmap.height)
end
...
function addnpc(tablenpc)
	...
	elseif tablenpc[1] == "pickupable" then
		table.insert(npcgroup, pickupable_init(tablenpc[2], tablenpc[3], tablenpc[4], tablenpc[5], tablenpc[6]))
	--warps
...
function enemy_ai()
	...
	for i=1,#npcgroup do
		...
		elseif npcgroup[i].id == "bat" then
			bat_ai(npcgroup[i])
		elseif npcgroup[i].id == "beastman_scout" then
			beastman_scout_ai(npcgroup[i])
		elseif npcgroup[i].id == "beastman_fighter" then
			beastman_fighter_ai(npcgroup[i])
		elseif npcgroup[i].id == "fireball" then
			fireball_ai(npcgroup[i])
